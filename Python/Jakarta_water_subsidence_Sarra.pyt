...
The main objective of this code is to process subsidence points obtained from SAR for their visualization. Given the large volume of dataset, we will experiment a few methods:

1. Use a wide format:
This approach uses the data structure as is and keeps date information in the wide format (along the column).
The disadvantage of this data structure is that the data cannot be filtered by dates.
...
## 1. Subtract the observed data of each date from the reference point
## Displacement data from all the Dates must be subtracted from a Reference Point.
## From the reformatted csv we will updated the observed data with the subtracted one.

# 2. Create XY points from CSV file
## 1.1. Add field names ('subdiv', 'district')

# 3. Add administrative boundary to the points
## 2.1. Subdivision
## 2.2. District

# 4. Run optimized hotspot analysis
...
xx. Use a long format (Aborted). no point of doing this.
This approach converts the original data from the wide format to a long format for date.
This evenatullay leads to more than 100 million points, which will exceed the capacity of our server and the maximum record limit of tables and portal for ArcGIS.
...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [CreateNewCSV, ExcelSubtract, InterQuartileRange, ToXYPoints,
                      AddAdminBoundaryToPoints, RunHotSpotAnalysisAddResults, ReformatScenarioExcel]
...
class ExcelSubtract(object):
    def __init__(self):
        self.label = "1. Subtract Observed Data from the Reference Point"
        self.description = "Subtract Observed Data from the Reference Point"
    ...
    def execute(self, params, messages):
        ...
        base_point = 1988267
        base_values = x.loc[x['id'] == base_point, years].values.flatten()
        x_sub = x[years].sub(base_values)

        nonyears = x.columns[x.columns.str.contains(r'deg$|^Disp|id',regex=True)]

        x1 = x[nonyears]
        x_updated = pd.merge(x1,x_sub,left_index=True,right_index=True)

        to_csv_file = os.path.join(directory, output_name + ".csv")
        x_updated.to_csv(to_csv_file, index=False)
    ...
    def execute(self, params, messages):
        ...
        for field in fields:
            ...
            row_to_append = pd.DataFrame(
                [{'dates': field, 'min': minv, 'Q1': q1, 'Q3': q3, 'max': maxv}])
...
class ToXYPoints(object):
    ...
    def execute(self, params, messages):
        ...
        arcpy.management.XYTableToPoint(
            input_table, out_feature_class, x_coords, y_coords, "", arcpy.SpatialReference(4326))  # default: 4326
...
class AddAdminBoundaryToPoints(object):
    ...
    def execute(self, params, messages):
        ...
        arcpy.management.AddField(sar_points_layer, subsiv_field_name,
                                  "TEXT", "", "", "", subsiv_field_name, "NULLABLE", "", "")
        ...
        for name in subdiv_names:
            ...
            arcpy.management.SelectLayerByAttribute(
                subdivision_boundary_layer, 'NEW_SELECTION', where_clause)

            # Select sar_points_layer by this location
            arcpy.management.SelectLayerByLocation(
                sar_points_layer, 'WITHIN', subdivision_boundary_layer)
...
class RunHotSpotAnalysisAddResults(object):
    ...
    def getParameterInfo(self):
        ...
        params = [input_layer, output_layer, analysis_field, join_field_sar]
        return params
    ...
    def execute(self, params, messages):
        ...
        arcpy.stats.OptimizedHotSpotAnalysis(
            input_layer, output_layer, analysis_field)
        ...
        if len(bools) == 1:
            arcpy.management.DeleteField(
                input_layer, [input_alias_field], "DELETE_FIELDS")
        arcpy.management.JoinField(
            input_layer, join_field_sar, output_layer, join_field_ohsa, [input_alias_field])
...
class ReformatScenarioExcel(object):
    ...
    def execute(self, params, messages):
        ...
        # Edit field names
        search_cols = x1.columns[x1.columns.str.contains(
            r'Time|^Obs|Calc', regex=True)]
        ...
        xx = pd.merge(left=x1, right=x2, how='left',
                      left_on=join_fields, right_on=join_fields)
        ...
        # Change date format
        xx[renamed_cols[0]] = pd.to_datetime(
            xx[renamed_cols[0]], errors='coerce').dt.date

        basename = os.path.splitext(os.path.basename(input_table))[0]

        to_excel_file = os.path.join(
            directory, basename + "_Scenario_compiled" + ".xlsx")
