# This R code reads Google sheet from a table provided by N2 Civil Team for monitoring Viaduct construction
# The source of URL: "https://docs.google.com/spreadsheets/d/11NMBpr1nKXuOgooHDl-CARvrieVN7VjEpLM1XgNwR0c/edit#gid=499307998&fvid=1497289736"
# 
# I copied the Google Sheet using IMPORTRANGE and created a copied Google SHeet (i.e., replica of the source)
# but with only necessary information
# GIS URL: "https://docs.google.com/spreadsheets/d/11YqYaenIB0l3Bpiv398-0QO3mEIR_BjvnMIsIOF3ILI/edit#gid=0"
# 
# OPERATION STEPS:
# 1. Read the Google Sheet
# 2. Restructure the data set
# 3. Join the table to our master list table
# 4. Export

# MAKE SURE THAT Type of Viaduct in Domain of ArcGIS Pro follows this:
# Bored Pile = 1
# Pile Cap = 2
# Pier Column = 3
# Pier Head = 4
# Precast = 5

# MAKE SURE THAT Status1 of Viaduct in Domain of ArcGIS Pro follows this:
# To be Constructed = 1
# Under Construction = 2
# Delayed = 3
# Completed = 4

library(googlesheets4)
library(openxlsx)
library(dplyr)
library(googledrive)
library(stringr)
library(reshape2)
library(fs)
library(lubridate)
library(rrapply)
library(purrr)

#******************************************************************#
## Enter Date of Update ##
date_update = "2024-07-17"

#******************************************************************#

gs4_auth(email="matsuzaki-ei@ocglobal.jp") #matsuzakieiji0@gmail.com
path =  path_home()

## Define working directory
wd = file.path(path, "Dropbox/01-Railway/02-NSCR-Ex/01-N2/03-During-Construction/02-Civil/03-Viaduct/01-Masterlist/02-Compiled")
setwd(wd)

# Read our master list table
## MasterList file name: "N2_Viaduct_MasterList.xlsx"
MLTable = file.choose()

# Read the masterlist:----
y = read.xlsx(MLTable)

############# BACKUP #################
## Skip if necessary
y$updated = as.Date(y$updated, origin = "1899-12-30")
y$updated = as.Date(y$updated, format="%m/%d/%y %H:%M:%S")
oldDate = gsub("-","",unique(y$updated))

fileName = paste(oldDate,"_",basename(MLTable),sep="")
direct = file.path(wd,"old")

write.xlsx(y,file.path(direct,fileName))
########## BACKUP END #################

##########************************************########
## The code below checks duplicated observations
## Run as necessary

#uType = unique(y$Type)
#uCP = unique(y$CP)

#temp = data.frame()
#for(i in uCP){
#  i="N-01"
#  for(j in uType){
#    j=2
#    yx = y[which(y$CP==i & y$Type==j),]
#    pierN = yx$nPierNumber
#    
#    dupPierN = pierN[duplicated(pierN)]
#
#    nrep = length(dupPierN)
#    t = data.frame(CP=rep(i,nrep),Type=rep(j,nrep),DupPierN=dupPierN)
#    temp = rbind(temp,t)
#  }
#}
#temp
#write.xlsx(temp,"N2_Duplicated_observations.xlsx")



###############################################################
####################### N-01 #################################:----
##############################################################

## Define URL where data is stored and updated
## I used "IMPORTRANGE" to copy information from the source URL to suit our needs.

url = "https://docs.google.com/spreadsheets/d/11NMBpr1nKXuOgooHDl-CARvrieVN7VjEpLM1XgNwR0c/edit?usp=sharing"


boredPiles = 2
pileC_pierC_pierH = 4
precast = 17

#################################
### N01: BORED PILES #############----
#################################

# 1. Read and write as CSV and xlsx
v = range_read(url, sheet = boredPiles)
v = data.frame(v)

# Extract only target field names
x = v[,c(3,12,22)]

colnames(x) = c("nPierNumber","end_date","Remarks")
x = x[-c(1,2),]

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
id = which(x$end_date < 1600000000)
x = x[-id,]
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 3. Edit nPierNumber:
## 3.1 Remove empty rows
rid = which(is.na(x$nPierNumber))
x = x[-rid,]

## 3.2. Keep bored piles for only viaduct
pile_id = which(str_detect(x$nPierNumber,"^P-|^P.*|^MT.*"))
x = x[pile_id,]


## 3.3.
###  Some piles failed for test and newly added to the list.
#"P370G Add"            "P378E1 (replacement)"
x$nPierNumber = gsub("-","",x$nPierNumber)

id = which(str_detect(x$nPierNumber, "Add|add|ADD|replacement|Replacement|REPLACEMENT"))
addPier = str_extract(x$nPierNumber[id],"P\\d+\\w+?") # any word before work break (space)


## Find exact match and casted
collapsedPier = paste0("^",addPier, "$", collapse = "|")
id1 = which(str_detect(x$nPierNumber,collapsedPier))

x$Status1 = 0

if(length(id1) > 0){
  x$Status1[id1] = 4
}

## Remove redundant (failed piers)
if (length(id) > 0) {
  x = x[-id,]
}


## 3.3. Remove hyphen from npierNumber
x$nPierNumber = gsub("-","",x$nPierNumber)

## 3.4. Detect duplicated nPierNumbers and if present delete
id = which(duplicated(x$nPierNumber))

if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 4. Edit Status1
## 4.1. Assign status numbers
x$Status1[str_detect(x$Remarks,pattern="Casted|casted")] = 4 # Bored Pile completed
x$Status1[str_detect(x$Remarks,pattern="Inprogress|inprogress|In-Progress|in-progress")] = 1 # Under Construction
x$Status1[str_detect(x$Remarks,pattern="Incomplete|incomplete")] = 1 # Delayed

x$Status1[which(x$Status1==0)]=1
id = which(is.na(x$Status1))
if(length(id) > 0) {
  x = x[-id,]
} else {
  print("No NA for Status1")
}

# 5. Add field names
x$CP = "N-01"
x$Type = 1


# P421H1 (X) -> P421H
id = which(str_detect(x$nPierNumber, "^P421H1$"))
x$nPierNumber[id] = gsub("H1", "H", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, "^P421H$"))
if(length(id) > 0) {
  x = x[-id, ]
}

# 5. Join new status to Viaduct masterlist
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

# 6. Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x.pile = x$nPierNumber[x$Status1==4]
yx.pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==1 & yx$CP.y=="N-01")]

missing_in_yx = x.pile[!x.pile %in% yx.pile] # not exist in yx table
missing_in_x = yx.pile[!yx.pile %in% x.pile] # not exist in x table

check_Completed = function(){
  if(length(missing_in_yx)>0){
    print("You have missing nPierNumber in yx table compared with x table")
  } else {
    if(length(missing_in_x)>0){
      print("YOu have missing nPierNumber in x table compared with yx table")
    }
  }
}
check_Completed()

# 7. Check for all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-01" & yx$Type==1)])

check = x_t %in% yx_t

check_function = function(){
  if(length(which(check=="FALSE"))>0){
    print("Number of Status1 is DIFFERENT. PLEASE CHECK")
  } else (
    print("GOOD! The number of Status1 is same between Civil and joined excel ML.")
  )
}

check_function()

# 8. Update using new data
gg = which(yx$CP.x=="N-01" & yx$Type==1)

## 8.1. end_date
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$end_date.x[gg] = yx$end_date.y[gg]

## 8.2. Status1
yx$Status1.x[gg] = yx$Status1.y[gg]

# 9. Delete fields and change status names
delField = which(str_detect(colnames(yx),"Status1.y|Remarks|CP.y|end_date.y"))
yx = yx[,-delField]

## Change field names
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
if(length(id) > 0) {
  yx$Status1[id] = 1
} else {
  print("no need to change Status1.")
}

# Check if empty status
id = nrow(yx[is.na(yx$Status1),])
if(id > 0) {
  print("There are rows missing Status1. Please check!!")
} else {
  print("Good. All rows have Status1. Please proceed.")
}

# Add date of updating table
## Delete old ones
iid = which(str_detect(colnames(yx),"updated|Updated|UPDATED"))
yx = yx[,-iid]
yx$updated = ymd(date_update)

# Overwrite MasterList
write.xlsx(yx, MLTable, overwrite = TRUE)


###################################################
# N-01: PILE CAP, PIER COLUN, and PIER HEAD:----
###################################################

## N-01 PILE CAP:-----
v = range_read(url, sheet = pileC_pierC_pierH)
v = data.frame(v)

x=v[,c(2,21,24)]
id = which(x[[1]] == "Mainline")
x=x[-c(1:id),]
colnames(x) = c("nPierNumber","end_date","Status1")

id=which(is.na(x$nPierNumber))
if(length(id) > 0) {
  x=x[-id,]
}

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. Edit end_date
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
x$Status1 = as.numeric(x$Status1)
x$Status1[which(x$Status1 == 1)] = 4

## 4.2. check for NA and remove if present
id = which(is.na(x$Status1) | x$Status1 < 1)
if(length(id) > 0) {
  x = x[-id,]
} else {
  print("No NA for Status1")
}


# 5. Add field names
x$CP = "N-01"
x$Type = 2

# 6. Re-format nPierNumber ###
id = which(str_detect(x$nPierNumber,"Center$|center$|Right$|right$|Left$|left$"))
id_extract = str_extract(x[id,"nPierNumber"],"Center$|center$|Right$|right$|Left$|left$")
del_w = unique(id_extract)
del_w = paste(del_w,collapse="|",sep="")

## 6.1. Delete these words from nPierNumber
x$nPierNumber[id] = gsub(del_w,"",x$nPierNumber[id])
unique(x$nPierNumber)

## 6.2. Conver to uppercase
x$nPierNumber = toupper(x$nPierNumber)

## 6.3. Remove space
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

## 6.4. Remove bracket if present
x$nPierNumber = gsub("[()]","",x$nPierNumber)

# 7. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==2 & yx$CP.y=="N-01")]

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
head(yx)
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-01" & yx$Type==2)])

check = x_t %in% yx_t
check_function()

# 8. Update table
## 8.1. end_date
gg = which(yx$CP.x=="N-01" & yx$Type==2)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$end_date.x[gg] = yx$end_date.y[gg]

## 8.2. Status1
yx$Status1.x[gg] = yx$Status1.y[gg]

## 8.3. Delete fields
delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert Status
id = which(is.na(yx$Status1))
if(length(id) > 0) {
  yx$Status1[id] = 1
} else {
  print("no need to change Status1.")
}
## 10.1. Check the presence of any empty Status1
yx[is.na(yx$Status1),]

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]

## Add new dates
yx$updated = ymd(date_update)

yx_pile = yx$nPierNumber[which(yx$Status1==4 & yx$Type==2 & yx$CP=="N-01")]

# 12. Export output
write.xlsx(yx, MLTable)


## N-01 PIER COLUMN:-----
x=v[,c(2,32,37)]
id = which(x[[1]] == "Mainline")
x=x[-c(1:id),]

colnames(x) = c("nPierNumber","end_date","Status1")

id=which(is.na(x$nPierNumber))
if(length(id) > 0) {
  x=x[-id,]
}


# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. Edit end_date
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
x$Status1 = as.numeric(x$Status1)
x$Status1[which(x$Status1 == 1)] = 4

## 4.2. check for NA and remove if present
id = which(is.na(x$Status1) | x$Status1 < 1)
if(length(id) > 0) {
  x = x[-id,]
} else {
  print("No NA for Status1")
}

# 5. Add field names
x$CP = "N-01"
x$Type = 3

# 6. Re-format nPierNumber ###
id = which(str_detect(x$nPierNumber,"Center$|center$|Right$|right$|Left$|left$"))
id_extract = str_extract(x[id,"nPierNumber"],"Center$|center$|Right$|right$|Left$|left$")
del_w = unique(id_extract)
del_w = paste(del_w,collapse="|",sep="")

## 6.1. Delete these words from nPierNumber
x$nPierNumber[id] = gsub(del_w,"",x$nPierNumber[id])
unique(x$nPierNumber)

## 6.2. Conver to uppercase
x$nPierNumber = toupper(x$nPierNumber)

## 6.3. Remove space
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

## 6.4. Remove bracket if present
x$nPierNumber = gsub("[()]","",x$nPierNumber)


# 7. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==3 & yx$CP.y=="N-01")]

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-01" & yx$Type==3)])

check = x_t %in% yx_t
check_function()

# 8. Update table
## 8.1. end_date
gg = which(yx$CP.x=="N-01" & yx$Type==3)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$end_date.x[gg] = yx$end_date.y[gg]

## 8.2. Status1
yx$Status1.x[gg] = yx$Status1.y[gg]

## 8.3. Delete fields
delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert Status
id = which(is.na(yx$Status1))
if(length(id) > 0) {
  yx$Status1[id] = 1
} else {
  print("no need to change Status1.")
}

## 10.1. Check the presence of any empty Status1
yx[is.na(yx$Status1),]

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]

## Add new dates
yx$updated = ymd(date_update)

# 12. Export output
write.xlsx(yx, MLTable)


## N-01 PIER HEAD:-----
x=v[,c(2,45,48)]
id = which(x[[1]] == "Mainline")
x=x[-c(1:id),]
colnames(x) = c("nPierNumber","end_date","Status1")

id=which(is.na(x$nPierNumber))
if(length(id) > 0) {
  x=x[-id,]
}


# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. Edit end_date
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
x$Status1 = as.numeric(x$Status1)
x$Status1[which(x$Status1 == 1)] = 4

## 4.2. check for NA and remove if present
id = which(is.na(x$Status1) | x$Status1 < 1)
if(length(id) > 0) {
  x = x[-id,]
} else {
  print("No NA for Status1")
}

# 5. Add field names
x$CP = "N-01"
x$Type = 4

# 6. Re-format nPierNumber ###
id = which(str_detect(x$nPierNumber,"Center$|center$|Right$|right$|Left$|left$"))
id_extract = str_extract(x[id,"nPierNumber"],"Center$|center$|Right$|right$|Left$|left$")
del_w = unique(id_extract)
del_w = paste(del_w,collapse="|",sep="")


## 6.1. Delete these words from nPierNumber
x$nPierNumber[id] = gsub(del_w,"",x$nPierNumber[id])
unique(x$nPierNumber)

## 6.2. Convert to uppercase
x$nPierNumber = toupper(x$nPierNumber)

## 6.3. Remove all spaces
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

## 6.4. Remove bracket if present
### for PierHead, there is a case where one PierHead supports two piers (SB/NB)
### N-01 data recorder uses P-169 (SB), P-169 (NB). But she only records P-169 (SB) for pier head
### As such, we need to find out this one and conver to 'P-169'
id = which(str_detect(x$nPierNumber, '[()]'))
x$nPierNumber[id] = gsub("[(NSB)]","",x$nPierNumber[id])

# 7. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==4 & yx$CP.y=="N-01")]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
head(yx)
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-01" & yx$Type==4)])

check = x_t %in% yx_t
check_function()

# 8. Update table
## 8.1. end_date
gg = which(yx$CP.x=="N-01" & yx$Type==4)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$end_date.x[gg] = yx$end_date.y[gg]

## 8.2. Status1
yx$Status1.x[gg] = yx$Status1.y[gg]

## 8.3. Delete fields
delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert Status
id = which(is.na(yx$Status1))
if(length(id) > 0) {
  yx$Status1[id] = 1
} else {
  print("no need to change Status1.")
}
## 10.1. Check the presence of any empty Status1
yx[is.na(yx$Status1),]

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]

## Add new dates
yx$updated = ymd(date_update)

# 12. Export output
write.xlsx(yx, MLTable)

###################################################
# N-01: Pre-CAST                        :----
###################################################

#url = "https://docs.google.com/spreadsheets/d/1Cl_tn3dTPdt8hrqzZtl_WjCxy9L-ltn7GrOmQpHdBw4/edit#gid=0"
#url = "https://docs.google.com/spreadsheets/d/11NMBpr1nKXuOgooHDl-CARvrieVN7VjEpLM1XgNwR0c/edit#gid=792494824"
# Read and write as CSV and xlsx
v = range_read(url, sheet = precast)
v = data.frame(v)

# Keep only fields needed
x = v[,c(3, 4,17,25)]

## colnames
colnames(x) = c("span","nPierNumber", "end_date", "Status1")
head(x)

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# identify the beginning and end of pier number
## Beginning
id = which(str_detect(x[[3]],"Actual Finished"))
x = x[c(id+1:nrow(x)),]

## End
id = max(which(str_detect(x[[1]],"Span|span")))
x = x[1:id,]
x = x[,-1]

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
x$Status1 = as.numeric(x$Status1)
id = which(x$Status1 == 1)
x = x[id,]
x$Status1[which(x$Status1 == 1)] = 4

## 4.2. check for NA and remove if present
id = which(is.na(x$Status1) | x$Status1 < 1)
if(length(id) > 0) {
  x = x[-id,]
} else {
  print("No NA for Status1")
}

# 5. Add field names
x$CP = "N-01"
x$Type = 5

# 6. Edit nPierNumber
x$nPierNumber = as.character(x$nPierNumber)
rem = unique(str_extract(x$nPierNumber,"\\([^()]+\\)"))

# if(!is.na(rem)){
#   ## 6.1. remove NA
#   rem = rem[!is.na(rem)]
#   rem = paste0(rem,collapse = "|")
#   
#   ## 6.2. remove from nPierNumber
#   id = which(str_detect(x$nPierNumber,rem))
#   x$nPierNumber[id] = gsub(rem,"",x$nPierNumber[id])
# }



## 6.3. Remove curly bracket
id=which(str_detect(x$nPierNumber,"[()]"))
x$nPierNumber[id] = gsub("[()]","",x$nPierNumber[id])

## 6.4. Remove all spaces
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

## 6.5. To Uppercase
x$nPierNumber = toupper(x$nPierNumber)

## 6.6 Re-format nPierNumber so that it aligns with the GIS attribute table
id_NB = which(str_detect(x$nPierNumber,"NB"))
id_SB = which(str_detect(x$nPierNumber,"SB"))

x$nPierNumber = str_extract(x$nPierNumber,"^P\\d+|MT.*")
x$nPierNumber[id_NB] = paste(x$nPierNumber[id_NB],"NB",sep = "")
x$nPierNumber[id_SB] = paste(x$nPierNumber[id_SB],"SB",sep = "")

x$nPierNumber = gsub("-","",x$nPierNumber)


# 7. Check Duplicated observation
id = which(duplicated(x$nPierNumber))

## 7.1. note that duplicated observations could simply be derived from wron
if(length(id)>0){
  ##xx = xx[-id,]
  print("There are duplicated nPierNumbers PLEASE CHCECK!!!")
} else {
  print("no duplicated observations")
}

# 8. Join new status to Viaduct masterlist
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-01" & yx$Type==5)])

check = x_t %in% yx_t
check_function()

x.precast = x$nPierNumber[x$Status1>=1]
yx.precast = yx$nPierNumber[which(yx$Type==5 & yx$CP.y=="N-01")]

missing_in_yx = x.precast[!x.precast %in% yx.precast] # not exist in yx table

# NA for status = 1 (To be Constructed)
str(yx)
gg = which(yx$CP.x=="N-01" & yx$Type==5)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$end_date.x[gg] = yx$end_date.y[gg]

## 8.2. Status1
yx$Status1.x[gg] = yx$Status1.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|start_date.y|CP.y|end_date.y"))
yx = yx[,-delField]

# Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]

## Add new dates
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# Output 
write.xlsx(yx, MLTable)

###############################################################
####################### N-02 #################################:----
##############################################################

#url = "https://docs.google.com/spreadsheets/d/1du9qnThdve1yXBv-W_lLzSa3RMd6wX6_NlNCz8PqFdg/edit?userstoinvite=junsanjose@gmail.com&actionButton=1#gid=0"
#url = "https://docs.google.com/spreadsheets/d/1du9qnThdve1yXBv-W_lLzSa3RMd6wX6_NlNCz8PqFdg/edit?usp=sharing"
url = "https://docs.google.com/spreadsheets/d/1du9qnThdve1yXBv-W_lLzSa3RMd6wX6_NlNCz8PqFdg/edit?usp=sharing"

# sheet no
sheetNumber = 4

###########################
### N-02: BORED PILES #:----
################################

# Read and write as CSV and xlsx
v = range_read(url, sheet = sheetNumber)
v = data.frame(v)

# Restruecture table
## Remove empty rows and unneeded rows
x = v[,c(1,5,10)]
colnames(x) = c("nPierNumber", "pileNo","end_date")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3.Identify the first and last
keep_row = which(str_detect(x$nPierNumber, "^P-|^MT"))
x = x[min(keep_row):nrow(x),]

# 4. Fill in pier numbers in the missing rows for each pier
id=which(!is.na(x$nPierNumber))
x$temp = 0
x$temp[id] = 1

## 4.1. Fill in missing piers
for(i in seq(nrow(x))){
  if(x$temp[i] == 0){
    x$nPierNumber[i] = x$nPierNumber[i-1]
  } else if(x$temp[i] == 1){
    x$nPierNumber[i] = x$nPierNumber[i]
  }
}

## 4.2. Remove unnecessary pier numbers
id=which(x$temp == 0)
x = x[id,]

## 4.3. Change pileNo format for N-02
x$pileNo = str_extract(x$pileNo,"\\d+")
x$temp = as.numeric(x$pileNo)

id=which(x$temp < 10)
x$pileNo[id] = paste("P0",x$temp[id],sep = "")

id=which(x$temp >= 10)
x$pileNo[id] = paste("P",x$temp[id],sep = "")

x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)
x$nPierNumber = gsub("P-","P",toupper(x$nPierNumber))
x$nPierNumber = paste(x$nPierNumber,"-",x$pileNo,sep = "")

# 5. Remove columns
x = x[,-c(2,4)]

# 6. Remove empty date
id=which(is.na(x$end_date))
x = x[-id,]

# 7. remove duplicated rows
id = which(duplicated(x$nPierNumber))

if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 8. Add field names
x$CP = "N-02"
x$Type = 1
x$Status1 = 4

# 9. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

##P596, P597, P598. Civil format is odd. I will remove N, S, NB, or SB
id = which(str_detect(x$nPierNumber, "^P59[6,7,8][S,SB,N,NB].*P\\d+"))
x$nPierNumber[id] = gsub("SB|NB|S|B|N","",x$nPierNumber[id])

# 10.Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==1 & yx$CP.y=="N-02")]

yx_pile[duplicated(yx_pile)]

str(x_pile)
str(yx_pile)

nn_x = str_extract(x_pile,"^P\\d+")
nn_x = gsub("P","",nn_x)
nn_x = unique(nn_x)

nn_y = str_extract(yx_pile,"^P\\d+")
nn_y = gsub("P","",nn_y)
nn_y = unique(nn_y)


missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-02" & yx$Type==1)])

check = x_t %in% yx_t
check_function()

# 8. Update using new data
gg = which(yx$CP.x=="N-02" & yx$Type==1)

## 8.1. end_date
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$end_date.x[gg] = yx$end_date.y[gg]

## 8.2. Status1
yx$Status1.x[gg] = yx$Status1.y[gg]

# 9. Delete fields and change status names
delField = which(str_detect(colnames(yx),"Status1.y|Remarks|end_date.y|CP.y"))
yx = yx[,-delField]

## Change field names
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
if(length(id) > 0) {
  yx$Status1[id] = 1
} else {
  print("no need to change Status1.")
}


# Check if empty status
id = nrow(yx[is.na(yx$Status1),])
if(id > 0) {
  print("There are rows missing Status1. Please check!!")
} else {
  print("Good. All rows have Status1. Please proceed.")
}

# Add date of updating table
## Delete old ones
iid = which(str_detect(colnames(yx),"updated|Updated|UPDATED"))
yx = yx[,-iid]
yx$updated = ymd(date_update)

# Overwrite MasterList
write.xlsx(yx, MLTable, overwrite = TRUE)


###################################################
# N-02: PILE CAP, PIER COLUN, and PIER HEAD:----
###################################################
## N-02 PILE CAP:-----
# Restruecture table
## Remove empty rows and unneeded rows
x = v[,c(1,38,40)]
colnames(x) = c("nPierNumber", "end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3.Identify the first and last
keep_row = which(str_detect(x$nPierNumber, "^P-|^MT"))
x = x[keep_row,]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Add Status1
x$Status1 = 4

# 5. remove duplicated
id = which(duplicated(x$nPierNumber))
if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 6. Edit nPierNumber
unique(x$nPierNumber)
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

x$nPierNumber[] = gsub("^P ","P",x$nPierNumber)
x$nPierNumber[] = gsub("P-","P",x$nPierNumber)

x$nPierNumber = gsub("LS","-LS",x$nPierNumber)
x$nPierNumber = gsub("RS","-RS",x$nPierNumber)

## S = SB, N = NB
id = which(str_detect(x$nPierNumber, "^P59[6,7][S]$"))
x$nPierNumber[id] = gsub("S$", "SB", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, "^P59[6,7][N]$"))
x$nPierNumber[id] = gsub("N$", "NB", x$nPierNumber[id])

## Civil Format is not accommodating GIS format
## P-598 -> P-598SB
id = which(str_detect(x$nPierNumber, "^P598$"))
x$nPierNumber[id] = "P598SB"

# 7. Add field names
x$CP = "N-02"
x$Type = 2

# 8. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

# 9. Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==2 & yx$CP.y=="N-02")]

# 10. check duplication
#dPileCap = x$nPierNumber[which(x$Type==2 & x$nPierNumber=="P1161-LS")]
x_pile[duplicated(x_pile)]
yx_pile[duplicated(yx_pile)]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# 11. check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-02" & yx$Type==2)])

check = x_t %in% yx_t
check_function()

# 12. Update table
gg = which(yx$CP.x=="N-02" & yx$Type==2)

yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")

yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 13. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 14. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 15. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 16. Export table 
write.xlsx(yx, MLTable)


######################
# N-02: PIER COLUMN:----
######################
x = v[,c(1,50,52)]
colnames(x) = c("nPierNumber", "end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3.Identify the first and last
keep_row = which(str_detect(x$nPierNumber, "^P-|^MT"))
x = x[keep_row,]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Add Status1
x$Status1 = 4

# 5. remove duplicated
id = which(duplicated(x$nPierNumber))
if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 6. Edit nPierNumber
unique(x$nPierNumber)
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

x$nPierNumber[] = gsub("^P ","P",x$nPierNumber)
x$nPierNumber[] = gsub("P-","P",x$nPierNumber)

x$nPierNumber = gsub("LS","-LS",x$nPierNumber)
x$nPierNumber = gsub("RS","-RS",x$nPierNumber)

## P-596, P-597. S -> SB, N -> NB
id = which(str_detect(x$nPierNumber, "^P59[6,7][S]$"))
x$nPierNumber[id] = gsub("S", "SB", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, "^P59[6,7][N]$"))
x$nPierNumber[id] = gsub("N", "NB", x$nPierNumber[id])

# 7. Add field names
x$CP = "N-02"
x$Type = 3

# 8. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

# 9. Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==3 & yx$CP.y=="N-02")]

# 10. check duplication
#dPileCap = x$nPierNumber[which(x$Type==2 & x$nPierNumber=="P1161-LS")]
x_pile[duplicated(x_pile)]
yx_pile[duplicated(yx_pile)]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# 11. check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-02" & yx$Type==3)])

check = x_t %in% yx_t
check_function()

# 12. Update table
gg = which(yx$CP.x=="N-02" & yx$Type==3)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")

yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 13. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 14. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 15. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 16. Export table 
write.xlsx(yx, MLTable)


######################
# N-02: PIER Head:----
######################
x = v[,c(1,62,64)]
colnames(x) = c("nPierNumber", "end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3.Identify the first and last
keep_row = which(str_detect(x$nPierNumber, "^P-|^MT"))
x = x[keep_row,]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}


# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Add Status1
x$Status1 = 4

# 5. remove duplicated
id = which(duplicated(x$nPierNumber))
if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 6. Edit nPierNumber
unique(x$nPierNumber)
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

x$nPierNumber[] = gsub("^P ","P",x$nPierNumber)
x$nPierNumber[] = gsub("P-","P",x$nPierNumber)

x$nPierNumber = gsub("LS","-LS",x$nPierNumber)
x$nPierNumber = gsub("RS","-RS",x$nPierNumber)

## P-596, P-597. S -> none, N -> none
id = which(str_detect(x$nPierNumber, "^P59[6,7,8][S]$|^P596SB$"))
x$nPierNumber[id] = gsub("S|SB", "", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, "^P59[6,7][N]$|^P598NB$"))
x$nPierNumber[id] = gsub("N|NB", "", x$nPierNumber[id])

### delete the duplicated if any
id = which(duplicated(x$nPierNumber))
if (length(id)>0) x = x[-id,]

# 7. Add field names
x$CP = "N-02"
x$Type = 4


# 8. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

# 9. Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==4 & yx$CP.y=="N-02")]

# 10. check duplication
#dPileCap = x$nPierNumber[which(x$Type==2 & x$nPierNumber=="P1161-LS")]
x_pile[duplicated(x_pile)]
yx_pile[duplicated(yx_pile)]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# 11. check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-02" & yx$Type==4)])

check = x_t %in% yx_t
check_function()

# 12. Update table
gg = which(yx$CP.x=="N-02" & yx$Type==4)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")

yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 13. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 14. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 15. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]


# 16. Export table 
write.xlsx(yx, MLTable)


###############################################################
####################### N-03 #################################:----
##############################################################
# original
#url = "https://docs.google.com/spreadsheets/d/19TBfdEpRuW7edwhiP7YdVSJQgrkot9wN7tDwzSTF76E/edit#gid=0"

url = "https://docs.google.com/spreadsheets/d/19TBfdEpRuW7edwhiP7YdVSJQgrkot9wN7tDwzSTF76E/edit?usp=sharing"
n03_pile = 1
n03_others = 2

############################################
###### N-03: Bored Piles #######:----
###########################################
# read and write as CSV and xlsx
v = range_read(url, sheet = n03_pile)
v = data.frame(v)

# filter out
x = v[,c(2,8,14)]
colnames(x) = c("nPierNumber", "end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

## 2.2. First and End 
id = which(x$Status1=="Status")
x = x[-c(1:id),]

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
## 4.1. Delete empty cells
rem_id = which(is.na(x$Status1))
if(length(rem_id)) {
  x = x[-rem_id,]
} else {
  print("No empty cells")
}

## 4.2. convert stats
x$Status1[str_detect(x$Status1,pattern="Completed|completed")] = 4 # Bored Pile completed
x$Status1[str_detect(x$Status1,pattern="Inprogress")] = 1 # Under Construction
x$Status1[str_detect(x$Status1,pattern="Incomplete")] = 1 # Delayed
x$Status1 = as.numeric(x$Status1)

# 5. Edit nPierNumber
pile_id = which(str_detect(x$nPierNumber,"^P-"))
x = x[pile_id,]

x$nPierNumber = gsub("^P-","P",x$nPierNumber)

# fix some bored pile numbers
id = which(str_detect(x$nPierNumber,"-0[0-9]$"))
x$nPierNumber[id] = gsub("-0", "-", x$nPierNumber[id])

# Note if pier number has these notations, we need to fix this. "P976R-4" = "P976-R4"
id = which(str_detect(x$nPierNumber, ".*L-"))
x$nPierNumber[id] = gsub("L-", "-L",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R-"))
x$nPierNumber[id] = gsub("R-", "-R",x$nPierNumber[id])

# Remove duplicated observations if any
id = which(duplicated(x$nPierNumber))

if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 6. Add CP and Type
x$CP = "N-03"
x$Type = 1

# 6. Read master list and join
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x.pile = x$nPierNumber[x$Status1==4]
yx.pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==1 & yx$CP.y=="N-03")]

missing_in_yx = x.pile[!x.pile %in% yx.pile] # not exist in yx table
missing_in_x = yx.pile[!yx.pile %in% x.pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-03" & yx$Type==1)])

check = x_t %in% yx_t
check_function()

# NA for status = 1 (To be Constructed)
gg = which(yx$CP.x=="N-03" & yx$Type==1)

yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx), "Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
if(length(id) > 1) {
  yx$Status1[id] = 1
} else {
  print("No NA for Status1")
}

# Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 
write.xlsx(yx, MLTable)



###################################################
# N-03: PILE CAP, PIER COLUN, and PIER HEAD:----
###################################################

## N-03 PILE CAP:-----
v = range_read(url, sheet = n03_others)
v = data.frame(v)

x=v[,c(1,20,23)]
id = which(str_detect(x[[3]],"Completed"))
x=x[id:nrow(x),]
colnames(x) = c("nPierNumber","end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. First and end
 id = which(x$Status1 == "Completed")
 x = x[-c(1:id),]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
id = which(str_detect(x$Status1, "%$"))
x$Status1[id] = gsub("%","",x$Status1[id])
x$Status1 = as.numeric(x$Status1)
x$Status1 = x$Status1/100

completed_row = which(x$Status1 == 1)
x$Status1[completed_row] = 4

# 5. Add field names
x$CP = "N-03"
x$Type = 2

# 6. Edit nPierNumber
# Extract only piles
pile_id = which(str_detect(x$nPierNumber,"^P-|^MT"))
x = x[pile_id,]
x$nPierNumber = gsub("^P-","P",x$nPierNumber)

id = which(str_detect(x$nPierNumber,"^MT"))
x$nPierNumber[id] = gsub("-","",x$nPierNumber[id])

x$nPierNumber = toupper(x$nPierNumber)

id = which(str_detect(x$nPierNumber, '^MT.*P[1,2]'))
if(length(id) > 0) {
  x$nPierNumber[id] = gsub('P','',x$nPierNumber[id])
}

# Note if pier number has these notations, we need to fix this. "P976R-4" = "P976-R4"
id = which(str_detect(x$nPierNumber, ".*L-"))
x$nPierNumber[id] = gsub("L-", "-L",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R-"))
x$nPierNumber[id] = gsub("R-", "-R",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*L"))
x$nPierNumber[id] = gsub("L", "-L", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R"))
x$nPierNumber[id] = gsub("R", "-R", x$nPierNumber[id])

# 7. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==2 & yx$CP.y=="N-03")]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-03" & yx$Type==2)])

check = x_t %in% yx_t
check_function()

# 8. Update table
gg = which(yx$CP.x=="N-03" & yx$Type==2)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 12. Export output
write.xlsx(yx, MLTable)


## N-03 PIER COLUMN:-----
x=v[,c(1,33,36)]
id = which(str_detect(x[[3]],"Completed"))
x=x[id:nrow(x),]

colnames(x) = c("nPierNumber","end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. First and end
id = which(x$Status1 == "Completed")
x = x[-c(1:id),]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
id = which(str_detect(x$Status1, "%$"))
x$Status1[id] = gsub("%","",x$Status1[id])
x$Status1 = as.numeric(x$Status1)
x$Status1 = x$Status1/100

completed_row = which(x$Status1 == 1)
x$Status1[completed_row] = 4

# 5. Add field names
x$CP = "N-03"
x$Type = 3

# 6. Edit nPierNumber
# Extract only piles
pile_id = which(str_detect(x$nPierNumber,"^P-|^MT"))
x = x[pile_id,]
x$nPierNumber = gsub("^P-","P",x$nPierNumber)

# Note if pier number has these notations, we need to fix this. "P976R-4" = "P976-R4"
id = which(str_detect(x$nPierNumber, ".*L-"))
x$nPierNumber[id] = gsub("L-", "-L",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R-"))
x$nPierNumber[id] = gsub("R-", "-R",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*L"))
x$nPierNumber[id] = gsub("L", "-L", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R"))
x$nPierNumber[id] = gsub("R", "-R", x$nPierNumber[id])

## civil: MT03-P2 -> GIS: MT032...
id = which(str_detect(x$nPierNumber, "^MT"))
x$nPierNumber[id] = gsub("-P", "", x$nPierNumber[id])

# 7. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==3 & yx$CP.y=="N-03")]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-03" & yx$Type==3)])

check = x_t %in% yx_t
check_function()

# 8. Update table
gg = which(yx$CP.x=="N-03" & yx$Type==3)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 12. Export output
write.xlsx(yx, MLTable)


## N-03 PIER Head:-----
x=v[,c(1,44,47)]
id = which(str_detect(x[[3]],"Completed"))
x=x[id:nrow(x),]

colnames(x) = c("nPierNumber","end_date","Status1")

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 3. First and end
id = which(x$Status1 == "Completed")
x = x[-c(1:id),]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Edit Status1
id = which(str_detect(x$Status1, "%$"))
x$Status1[id] = gsub("%","",x$Status1[id])
x$Status1 = as.numeric(x$Status1)
x$Status1 = x$Status1/100

completed_row = which(x$Status1 == 1)
x$Status1[completed_row] = 4

# 5. Add field names
x$CP = "N-03"
x$Type = 4

# 6. Edit nPierNumber
# Extract only piles
pile_id = which(str_detect(x$nPierNumber,"^P-"))
x = x[pile_id,]
x$nPierNumber = gsub("^P-","P",x$nPierNumber)

# Note if pier number has these notations, we need to fix this. "P976R-4" = "P976-R4"
id = which(str_detect(x$nPierNumber, ".*L-"))
x$nPierNumber[id] = gsub("L-", "-L",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R-"))
x$nPierNumber[id] = gsub("R-", "-R",x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*L"))
x$nPierNumber[id] = gsub("L", "-L", x$nPierNumber[id])

id = which(str_detect(x$nPierNumber, ".*R"))
x$nPierNumber[id] = gsub("R", "-R", x$nPierNumber[id])

# P1120, P1121 has only one structure. If civil GS has two such as -L or -R,
# we need to fix this to join to GIS table
id = which(str_detect(x$nPierNumber,"^P1120|^P1121"))
x$nPierNumber[id] = gsub('-L|-R','',x$nPierNumber[id])

id = which(duplicated(x$nPierNumber))
x = x[-id,]

# 7. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==4 & yx$CP.y=="N-03")]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-03" & yx$Type==4)])

check = x_t %in% yx_t
check_function()

# 8. Update table
gg = which(yx$CP.x=="N-03" & yx$Type==4)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 12. Export output
write.xlsx(yx, MLTable)


## N-03 precast: ----
v = range_read(url, sheet = 3)
v = data.frame(v)

x=v[,c(1,20,22)]
id = which(str_detect(x[[3]],"yes|Yes|YES"))
ids = id+1
idd = sort(c(id, ids))
x=x[idd,]

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}
colnames(x) = c("nPierNumber","end_date","Status1")
piers = x$nPierNumber[which(!is.na(x$nPierNumber))]
status = x$Status1[which(!is.na(x$Status1))]
id = which(is.na(x$nPierNumber) & is.na(x$end_date) & is.na(x$Status1))
id2 = which(!str_detect(x$end_date, '^A|NA'))
idd = sort(c(id,id2))
dates = x$end_date[idd]

x = data.frame(nPierNumber=piers, end_date=dates, Status1=status)

##########################################################

###############################################################################
# WORK IN PROGRESS
# identify missing end-date
head(x,90)
id = which(is.na(x$nPierNumber) & is.na(x$end_date) & is.na(x$Status1))
piers_for_dates = x$nPierNumber[id - 1]
search_piers = paste0("^",piers_for_dates,"$",collapse="|")
id1 = which(str_detect(x$nPierNumber,search_piers))
id2 = id1 - 1
idd = sort(c(id1, id2))
x1 = x[idd,]
x1 = x[which(str_detect(x$nPierNumber,search_piers)),]

x2=v[,c(1,10:20,23)]
x2 = x2[which(str_detect(x2[[1]], search_piers)),]

###############################################################################



# 3. First and end
x$Status1 = tolower(x$Status1)
id = which(x$Status1 == unique(x$Status1))
x = x[id,]

# 3. Delete empty 
id=which(is.na(x$Status1))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# reformat nPierNumber
x$nPierNumber = gsub('-.*','',x$nPierNumber)
x$Type = 5
x$Status1 = 4
x$CP = 'N-03'

id = which(x$nPierNumber == x$nPierNumber[duplicated(x$nPierNumber)])
if(length(id) > 0) {
  x = x[-id[1], ]
}

y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==5 & yx$CP.y=="N-03")]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-03" & yx$Type==5)])

check = x_t %in% yx_t
check_function()

# 8. Update table
gg = which(yx$CP.x=="N-03" & yx$Type==5)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 9. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 10. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 11. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

# 12. Export output
write.xlsx(yx, MLTable)


###############################################################
####################### N-04 #################################:----
##############################################################
#boredPile = 4
pCap_pCol_pHead = 5

#url = "https://docs.google.com/spreadsheets/d/1OWdmM36PWL5MgH0lK9HpigaoVq4L7Q6hm7DSN2FxiAA/edit#gid=0"
url = "https://docs.google.com/spreadsheets/d/1ospNZHCvk2D35zB9psxkOczCExCfOvcp4skYQbDO7n4/edit?usp=sharing"

# ### N-04: BORED PILES #:----
# # Read and write as CSV and xlsx
# v = range_read(url, sheet = boredPile)
# v = data.frame(v)
# 
# # Restruecture table
# ## I temporarliy used dummy field names to be discarded so need to remove it
# x = v[,c(6,19,30)]
# colnames(x) = c("nPierNumber","end_date","Status1")
# 
# # 2. Check the presence of columns in the format of 'list'
# coln = colnames(x)
# for(i in seq(coln)) {
#   type = class(x[[i]])
#   
#   # if type is 'list', unlist
#   if(type == "list") {
#     ## Convert NULL to NA
#     x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
#     
#     ## then unlist
#     x[[i]] = unlist(x[[i]], use.names = FALSE)
#   }
# }
# 
# # 3. first and end
# id = which(x$Status1 == "Status")
# x = x[-c(1:id),]
# x = x[str_detect(x$nPierNumber,"^P|^PLK|^DEP"),]
# 
# # 4. Remove empty Status1
# rid = which(is.na(x$Status1))
# 
# if(length(rid)==0){
#   print("no nrows being removed")
# } else {
#   x = x[-rid,]
# }
# 
# # 3. Edit end_date
# x$end_date = as.numeric(x$end_date)
# x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
# x$end_date = as.Date(x$end_date, origin = "1899-12-30")
# x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")
# 
# # 4. Edit nPierNumber
# # Remove hyphen from pierNumber
# ## correct with "P-"
# x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)
# #x$nPierNumber = gsub("^[P]","P-",x$nPierNumber)
# #x$nPierNumber = gsub("^PLK ","PLK-",x$nPierNumber)
# x$nPierNumber = gsub("DEPO","DEP0",x$nPierNumber)
# 
# ## Fix some wrong notation of nPierNumber
# id_LS = which(str_detect(x$nPierNumber,'LS'))
# id_RS = which(str_detect(x$nPierNumber,'RS'))
# 
# x$nPierNumber[id_LS] = gsub("LS-01","-1LS",x$nPierNumber[id_LS])
# x$nPierNumber[id_LS] = gsub("LS-02","-2LS",x$nPierNumber[id_LS])
# x$nPierNumber[id_LS] = gsub("LS-03","-3LS",x$nPierNumber[id_LS])
# 
# #x$nPierNumber[id_LS] = gsub("01LS","1LS",x$nPierNumber[id_LS])
# #x$nPierNumber[id_LS] = gsub("02LS","2LS",x$nPierNumber[id_LS])
# 
# x$nPierNumber[id_RS] = gsub("RS-01","-1RS",x$nPierNumber[id_RS])
# x$nPierNumber[id_RS] = gsub("RS-02","-2RS",x$nPierNumber[id_RS])
# x$nPierNumber[id_RS] = gsub("RS-03","-3RS",x$nPierNumber[id_RS])
# 
# #x$nPierNumber[id_RS] = gsub("01RS","1RS",x$nPierNumber[id_RS])
# #x$nPierNumber[id_RS] = gsub("02RS","2RS",x$nPierNumber[id_RS])
# 
# # 5. Edit Status1
# x$Status1[str_detect(x$Status1,pattern="Casted|casted")] = 4 # Bored Pile completed
# x$Status1[str_detect(x$Status1,pattern="Inprogress")] = 1 # Under Construction
# x$Status1[str_detect(x$Status1,pattern="Incomplete")] = 1 # Delayed
# x$Status1 = as.numeric(x$Status1)
# 
# ## 5.1. remove emtpy status1 if any
# id = which(is.na(x$Status1))
# if(length(id) > 0) {
#   x = x[-id,]
# } else {
#   print("No empty Status1")
# }
# 
# # 6. Remove duplicated observations if any
# id = which(duplicated(x$nPierNumber))
# 
# if(length(id)>0){
#   x = x[-id,]
# } else {
#   print("no duplicated observations")
# }
# 
# # 7. Add field names
# x$CP = "N-04"
# x$Type = 1
# 
# # 7. Join
# y = read.xlsx(MLTable)
# yx = left_join(y,x,by=c("nPierNumber","Type"))
# 
# ## Check if the number of Status1 for each Type is same between x and yx.
# ## Check for piles with completed status
# x.pile = x$nPierNumber[x$Status1==4]
# yx.pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==1 & yx$CP.y=="N-04")]
# 
# missing_in_yx = x.pile[!x.pile %in% yx.pile] # not exist in yx table
# missing_in_x = yx.pile[!yx.pile %in% x.pile] # not exist in x table
# 
# check_Completed()
# 
# # check all statuses
# x_t = table(x$Status1)
# yx_t = table(yx$Status1.y[which(yx$CP.y=="N-04" & yx$Type==1)])
# 
# check = x_t %in% yx_t
# check_function() 
# 
# # 8.
# gg = which(yx$CP.x=="N-04" & yx$Type==1)
# 
# yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
# yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")
# 
# yx$Status1.x[gg] = yx$Status1.y[gg]
# yx$end_date.x[gg] = yx$end_date.y[gg]
# 
# delField = which(str_detect(colnames(yx), "Status1.y|Remarks|CP.y|end_date.y"))
# yx = yx[,-delField]
# 
# # Change Status name
# colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
# colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
# colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"
# 
# # Convert nA to 1 (to be Constructed)
# id = which(is.na(yx$Status1))
# yx$Status1[id] = 1
# 
# # Add date of updating table
# ## Delete old ones
# iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
# yx = yx[,-iid]
# yx$updated = ymd(date_update)
# 
# yx[is.na(yx$Status1),]
# 
# # 
# write.xlsx(yx, MLTable)

###################################################
### N-04: Pilec Caps, Pier, and Pier Head #;----
###################################################
## N-04 PILE CAP:-----
v = range_read(url, sheet = pCap_pCol_pHead)
v = data.frame(v)

x=v[,c(2,15)]

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

colnames(x) = c("nPierNumber","end_date")

# 3. First and end
id = which(str_detect(x$end_date, "Completed$"))
x = x[-c(1:id),]

# 4. Delete empty 
id=which(is.na(x$end_date) | is.na(x$nPierNumber))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}


# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Add Status1
x$Status1 = 4

# 5. remove duplicated
id = which(duplicated(x$nPierNumber))
x$nPierNumber[id]
if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 6. Edit nPierNumber
unique(x$nPierNumber)
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

x$nPierNumber[] = gsub("^P ","P",x$nPierNumber)
x$nPierNumber[] = gsub("P-","P",x$nPierNumber)

x$nPierNumber = gsub("LS","-LS",x$nPierNumber)
x$nPierNumber = gsub("RS","-RS",x$nPierNumber)

# 7. Add field names
x$CP = "N-04"
x$Type = 2

# 8. Join 
head(y)
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

# 9. Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==2 & yx$CP.y=="N-04")]

# 10. check duplication
#dPileCap = x$nPierNumber[which(x$Type==2 & x$nPierNumber=="P1161-LS")]
x_pile[duplicated(x_pile)]
yx_pile[duplicated(yx_pile)]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# 11. check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-04" & yx$Type==2)])

check = x_t %in% yx_t
check_function()

# 12. Update table
gg = which(yx$CP.x=="N-04" & yx$Type==2)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")

yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 13. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 14. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 15. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]


# 16. Export table 
write.xlsx(yx, MLTable)


## N-04 PIER COLUMN:-----
x=v[,c(18,20,21,22,24,25,26,29)]

coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

# 1st lift
firstRow = which(str_detect(x[,1],"Date Completed"))
x1 = x[-c(1:firstRow),c(1:3)]
colnames(x1) = c("end_date", "week", "nPierNumber")

## Find where 'week' and 'nPierNumber' are both filled
id=which(!is.na(x1$week) & !is.na(x1$nPierNumber))
x1 = x1[id, c(1,3)]

# 2nd lift
x2 = x[-c(1:firstRow),c(4:6)]
colnames(x2) = c("end_date", "week", "nPierNumber")
id=which(!is.na(x2$week) & !is.na(x2$nPierNumber))
x2 = x2[id, c(1,3)]

# 3rd lift
x3 = x[-c(1:firstRow),c(7:8)]
colnames(x3) = c("end_date", "nPierNumber")
id=which(!is.na(x3$nPierNumber))
x3 = x3[id,]

# Compile x1, x2, and x3
xx = rbind(x1, x2, x3)

# Extract only piles
# Re-format nPierNumber
xx$nPierNumber = gsub("[[:space:]]","",xx$nPierNumber)

xx$nPierNumber[] = gsub("^P ","P",xx$nPierNumber)
xx$nPierNumber[] = gsub("P-","P",xx$nPierNumber)

xx$nPierNumber = gsub("LS","-LS",xx$nPierNumber)
xx$nPierNumber = gsub("RS","-RS",xx$nPierNumber)

# remove duplicated
id = which(duplicated(xx$nPierNumber))
if(length(id)>0){
  xx = xx[-id,]
} else {
  print("no duplicated observations")
}

# Add Status1
xx$Status1 = 4

# convert date to numeric
xx$end_date = as.numeric(xx$end_date)

xx$CP = "N-04"

# date
xx$end_date = as.POSIXlt(xx$end_date, origin="1970-01-01",tZ="UTC")
xx$end_date = as.Date(xx$end_date, origin = "1899-12-30")
xx$end_date = as.Date(xx$end_date, format="%m/%d/%y %H:%M:%S")

# type
xx$Type = 3

# Join 
y = read.xlsx(MLTable)
yx = left_join(y,xx,by=c("nPierNumber","Type"))

## Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = xx$nPierNumber[which(xx$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==3 & yx$CP.x=="N-04")]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# check all statuses
x_t = table(xx$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.x=="N-04" & yx$Type==3)])

check = x_t %in% yx_t
check_function()

# NA for status = 1 (To be Constructed)
#gg = which(yx$Status1.y>0)

gg = which(yx$CP.x=="N-04" & yx$Type==3)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")

yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

str(yx)
delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]

## Add new dates
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]
write.xlsx(yx, MLTable)


## N-04 PIER HEAD:-----
x=v[,c(2,31)]

# 2. Check the presence of columns in the format of 'list'
coln = colnames(x)
for(i in seq(coln)) {
  type = class(x[[i]])
  
  # if type is 'list', unlist
  if(type == "list") {
    ## Convert NULL to NA
    x[[i]] = rrapply(x[[i]], f = function(x) replace(x, is.null(x), NA))
    
    ## then unlist
    x[[i]] = unlist(x[[i]], use.names = FALSE)
  }
}

colnames(x) = c("nPierNumber","end_date")

# 3. First and end
id = which(str_detect(x$end_date, "Completed$"))
x = x[-c(1:id),]

# 4. Delete empty 
id=which(is.na(x$end_date))
if(length(id) > 0) {
  x=x[-id,]
} else {
  print("No empty Status1")
}

# remove emtpy nPierNumber
id = which(is.na(x$nPierNumber))
if (length(id) > 0){
  x = x[-id,]
}

# 3. Edit end_date
x$end_date = as.numeric(x$end_date)
x$end_date = as.POSIXlt(x$end_date, origin="1970-01-01",tZ="UTC")
x$end_date = as.Date(x$end_date, origin = "1899-12-30")
x$end_date = as.Date(x$end_date, format="%m/%d/%y %H:%M:%S")

# 4. Add Status1
x$Status1 = 4

# 5. remove duplicated
id = which(duplicated(x$nPierNumber))
x$nPierNumber[id]
if(length(id)>0){
  x = x[-id,]
} else {
  print("no duplicated observations")
}

# 6. Edit nPierNumber
unique(x$nPierNumber)
x$nPierNumber = gsub("[[:space:]]","",x$nPierNumber)

x$nPierNumber[] = gsub("^P ","P",x$nPierNumber)
x$nPierNumber[] = gsub("P-","P",x$nPierNumber)

x$nPierNumber = gsub("LS","-LS",x$nPierNumber)
x$nPierNumber = gsub("RS","-RS",x$nPierNumber)


## P-1161
id=which(str_detect(x$nPierNumber,"P1159|P1160|P1161|P1162"))
x$nPierNumber[id] = gsub("-LS|RS", "", x$nPierNumber[id])

# 7. Add field names
x$CP = "N-04"
x$Type = 4

# 8. Join 
y = read.xlsx(MLTable)
yx = left_join(y,x,by=c("nPierNumber","Type"))

# 9. Check if the number of Status1 for each Type is same between x and yx.
## Check for piles with completed status
x_pile = x$nPierNumber[which(x$Status1==4)]
yx_pile = yx$nPierNumber[which(yx$Status1.y==4 & yx$Type==4 & yx$CP.y=="N-04")]

# 10. check duplication
#dPileCap = x$nPierNumber[which(x$Type==2 & x$nPierNumber=="P1161-LS")]
x_pile[duplicated(x_pile)]
yx_pile[duplicated(yx_pile)]

str(x_pile)
str(yx_pile)

missing_in_yx = x_pile[!x_pile %in% yx_pile] # not exist in yx table
missing_in_x = yx_pile[!yx_pile %in% x_pile] # not exist in x table

check_Completed()

# 11. check all statuses
x_t = table(x$Status1)
yx_t = table(yx$Status1.y[which(yx$CP.y=="N-04" & yx$Type==4)])

check = x_t %in% yx_t
check_function()

# 12. Update table
gg = which(yx$CP.x=="N-04" & yx$Type==4)
yx$end_date.x = as.Date(yx$end_date.x, origin = "1899-12-30")
yx$end_date.x = as.Date(yx$end_date.x, format="%m/%d/%y %H:%M:%S")

yx$Status1.x[gg] = yx$Status1.y[gg]
yx$end_date.x[gg] = yx$end_date.y[gg]

delField = which(str_detect(colnames(yx),"Status1.y|CP.y|end_date.y"))
yx = yx[,-delField]

# 13. Change Status name
colnames(yx)[str_detect(colnames(yx),pattern="Status1")] = "Status1"
colnames(yx)[str_detect(colnames(yx),pattern="CP")] = "CP"
colnames(yx)[str_detect(colnames(yx),pattern="end_date")] = "end_date"

# 14. Convert nA to 1 (to be Constructed)
id = which(is.na(yx$Status1))
yx$Status1[id] = 1

# 15. Add date of updating table
## Delete old ones
iid = which(colnames(yx) %in% colnames(yx)[str_detect(colnames(yx),"updated|Updated|UPDATED")])
yx = yx[,-iid]
yx$updated = ymd(date_update)

yx[is.na(yx$Status1),]

## Finally, Convert planned_date to date format
yx$planned_date = as.Date(yx$planned_date, origin = "1899-12-30")
yx$planned_date  = as.Date(yx$planned_date, format="%m/%d/%y %H:%M:%S")

# 16. Export table 
write.xlsx(yx, MLTable)

###############################################################
####################### Final Data Prepa #####################:----
##############################################################
# If planned_date < current date & Status1 is NOT complete, Status1 = 3 (delayed)
# today = Sys.Date()
# id = which(yx$planned_date < today & yx$Status1 != 4)
# yx$Status1[id] = 3 # delayed status
# 
# yx$end_date = as.Date(yx$end_date, origin = "1899-12-30")
# yx$end_date = as.Date(yx$end_date, format="%m/%d/%y %H:%M:%S")
# yx$updated = as.Date(yx$updated, origin = "1899-12-30")
# yx$updated = as.Date(yx$updated, format="%m/%d/%y %H:%M:%S")
# 
# write.xlsx(yx, MLTable)
################################################################
###
# summary table to check
y = read.xlsx(MLTable)
y1 = y[which(y$Status1 == 4),]

##agg_df <- aggregate(y1$Status1, by=list(y1$CP, y1$Type, y1$Year, y1$Status1), FUN=length)
agg_df <- aggregate(y1$Status1, by=list(y1$CP, y1$Type, y1$Status1), FUN=length)
agg_df

##
agg_df <- aggregate(y1$Status1, by=list(y1$Year, y1$Month, y1$Type), FUN=length)
agg_df

ttt = agg_df[order(agg_df$Group.1, agg_df$Group.2, agg_df$Group.3),]

## choose your own files and calculate summary
a = file.choose()
y = read.xlsx(a)
y1 = y[which(y$Status1 == 4),]
agg_df <- aggregate(y1$Status1, by=list(y1$CP, y1$Type, y1$Status1), FUN=length)
agg_df


###
a = file.choose()
gis = read.xlsx(a)
id=which(gis$H_Level == 2)
gis_p = gis[id,]
unique(gis_p$H_Level)


b = file.choose()
env = read.xlsx(b,startRow = 2)
id=which(env$High_Level=='Pending Delivery')
env_p = env[id,]

str(gis_p)
id_gis = gis_p$Id
id_env = env_p$Id

id_gis[!id_gis %in% id_env]
id_env[!id_env %in% id_gis]


?read.xlsx
head(env)
agg = aggregate(gis$H_Level, by=list(gis$H_Level,gis$Package),FUN=length)
agg


###
a = file.choose()
gis = read.xlsx(a)

b = file.choose()
env = read.xlsx(b,startRow = 2)

c = file.choose()
gis_por = read.xlsx(c)

str(env)
# StatusNVS3
gis_sum <- aggregate(gis$StatusNVS3, by=list(gis$Package,gis$StatusNVS3), FUN=length)
gis_portal_sum <- aggregate(gis_por$StatusNVS3, by=list(gis_por$Package,gis_por$StatusNVS3), FUN=length)
env_sum <- aggregate(env$Status.NVS, by=list(env$Package,env$Status.NVS), FUN=length)

statusNVS3_table = data.frame(gis_ml=gis_sum[,c(1,3)],gis_portal=gis_portal_sum[,3],env_ml=env_sum[,3])

# HandedOver
gis_sum <- aggregate(gis$HandedOver, by=list(gis$Package,gis$HandedOver), FUN=length); gis_sum=gis_sum[which(gis_sum$Group.2 == 1),]
gis_portal_sum = aggregate(gis_por$HandedOver, by=list(gis_por$Package,gis_por$HandedOver), FUN=length); gis_portal_sum=gis_portal_sum[which(gis_portal_sum$Group.2 == 1),]
env_sum <- aggregate(env$Handed.over.to.JV, by=list(env$Package,env$Handed.over.to.JV), FUN=length); env_sum=env_sum[which(env_sum$Group.2 == 1),]

handedOver1_table = data.frame(gis_ml=gis_sum[,c(1,3)],gis_portal=gis_portal_sum[,3],env_ml=env_sum[,3])

# PTE
gis_sum <- aggregate(gis$PTE, by=list(gis$Package, gis$PTE), FUN=length); gis_sum=gis_sum[which(gis_sum$Group.2 == 1),]
gis_portal_sum <- aggregate(gis_por$PTE, by=list(gis_por$Package,gis_por$PTE), FUN=length); gis_portal_sum=gis_portal_sum[which(gis_portal_sum$Group.2 == 1),]
env_sum <- aggregate(env$Lot.with.PTE, by=list(env$Package,env$Lot.with.PTE), FUN=length); env_sum=env_sum[which(env_sum$Group.2 == 1),]

pte1_table = data.frame(gis_ml=gis_sum[,c(1,3)],gis_portal=gis_portal_sum[,3],env_ml=env_sum[,3])


# check Ids between GIS ML and GIS Attribute table
str(gis_por)
gis_ids = gis$Id
gis_por_ids = gis_por$Id

miss_ids_gis_portal = gis_ids[!gis_ids %in% gis_por_ids] # missing Ids in portal
miss_id_names = paste0(miss_ids_gis_portal,collapse = "|")
ids = which(str_detect(gis$Id,miss_id_names))
gis[ids,c('Package','Id','HandedOver','PTE',"StatusNVS3")]

ids=which(str_detect(env$Id,miss_id_names))
env[ids,c('Package','Id','Handed.over.to.JV','Lot.with.PTE',"Status.NVS")]


# statusNVS3 = 4
gis_4_id = gis[which(gis$StatusNVS3 == 4),c('Id')]
env_4_id = env[which(env$Status.NVS == 4),c('Id')]
gis_4_id[!gis_4_id %in% env_4_id]
env_4_id[!env_4_id %in% gis_4_id]
